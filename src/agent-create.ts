...
1. **The dashboard's per-agent avatar uploader** (Agents tab → click your card → camera icon on the avatar). That sets the avatar shown inside HansCorp (sidebar, mission control, war room) — NOT the one on Telegram.
...
function generateSystemdUnit(agentId: string): string {
  ...
  const unit = `[Unit]
Description=HansCorp Agent: ${agentId}
...
export function suggestBotNames(agentId: string): { displayName: string; username: string } {
  ...
  return {
    displayName: `HansCorp ${label}`,
    username: `hanscorp_${agentId.replace(/-/g, '_')}_bot`,
  };
}
