          ...
          >
            <div class="flex items-center gap-2 mb-1">
              ...
              <span class="font-mono text-[10.5px] text-[var(--color-text-muted)]">
                <span data-agent-id-tooltip={`Agent ID: ${hoveredEntry.agent_id}`} title={`Agent ID: ${hoveredEntry.agent_id}`}>
                  {agentHandle(hoveredEntry.agent_id)}
                </span> · {hoveredEntry.action}
              </span>
  ...
  return (
    <>
      <header class="flex items-center px-4 py-3 border-b border-[var(--color-border)] gap-2">
        ...
        <span class="font-mono text-[12px] text-[var(--color-text)]" data-agent-id-tooltip={`Agent ID: ${entry.agent_id}`} title={`Agent ID: ${entry.agent_id}`}>{agentHandle(entry.agent_id)}</span>
        {lobeLabel && (
            ...
            {Object.entries(visibleAgents).sort((a, b) => b[1] - a[1]).map(([id, count]) => {
              ...
              return (
                ...
                >
                  ...
                  <span class={'font-mono text-[11.5px] ' + (on ? 'text-[var(--color-text)]' : 'text-[var(--color-text-faint)]')} data-agent-id-tooltip={`Agent ID: ${id}`} title={`Agent ID: ${id}`}>{agentHandle(id)}</span>
          ...
          <div class="flex-1 space-y-1 min-w-0">
            {slices.slice(0, 4).map(([agentId, count]) => (
              <div key={agentId} class="flex items-center gap-1.5 text-[10.5px]">
                ...
                />
                <span class="font-mono truncate text-[var(--color-text-muted)]" data-agent-id-tooltip={`Agent ID: ${agentId}`} title={`Agent ID: ${agentId}`}>{agentHandle(agentId)}</span>
