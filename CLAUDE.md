...
**IMPORTANT:** The app root is `/Users/hernan/Git/claudeclaw-os`. Do not use `~/.claudeclaw` as the app root; that directory only stores external config. **Never use `find` to locate schedule-cli.js** as it will search your entire home directory and hang.
