...
HansCorp is not a chatbot wrapper. It spawns the actual `claude` CLI on your Mac, Linux, or Windows machine and pipes the result back to your Telegram chat. Everything that works in your terminal (your skills, your tools, your context) works from your phone.

![HansCorp at a glance](assets/claudeclaw-overview.png)
...
HansCorp has two tiers of features. The **core** features work out of the box with just a Telegram bot token. The **experimental** features are opt-in and require additional setup.
...
![HansCorp setup flow](assets/setup-flow.jpeg)
...
**macOS users:** After starting HansCorp for the first time, your Mac may show "Node wants to access..." permission dialogs. You need to click Allow on each one or the bot will silently hang. Keep an eye on your Mac screen during the first run.

**Which Claude plan works best?** HansCorp runs the `claude` CLI, so any plan works (Free, Pro, Max). However, complex multi-step tasks (building skills, debugging code, multi-agent work) perform significantly better on **Opus**: If you're on the Free or Pro plan and Claude struggles with a task, the model matters. Sonnet is fast but often can't handle the kind of agentic work HansCorp enables. Max ($100 or $200) with Opus is the recommended experience.
...
You need a bot token from Telegram. This is what HansCorp uses to send and receive messages.
...
> ```
> I just cloned HansCorp. Please read README.md and set me up completely.
...
You probably want HansCorp running automatically, not manually in a terminal.
...
- **WSL2 (recommended)**: `wsl --install -d Ubuntu` in an elevated PowerShell, reboot, clone HansCorp *inside* the Ubuntu filesystem (not `/mnt/c`), then follow the Linux steps above. Keep `~/.claude/` inside WSL2.
...
```bash
cd claudeclaw-os       # go to your HansCorp directory
...
![HansCorp architecture](assets/architecture.png)
...
> HansCorp ships with bundled Gmail and Google Calendar skills that work great out of the box. This is an **optional alternative** if you want broader Google Workspace access from a single tool.
...
| **Skill** | A markdown file (`.md`) that teaches Claude how to do something specific, usually by combining CLI commands, API calls, or code into a workflow. | Claude reads the skill file and follows its instructions. HansCorp's bundled Gmail skill, for example, tells Claude which Python scripts to run and how to format the output. |
...
HansCorp downloads the video to `workspace/uploads/` and tells Claude to analyze it with the `gemini-api-dev` skill. Without `GOOGLE_API_KEY`, Claude receives the file path but can't understand the content. Telegram caps downloads at 20MB.
...
**How it works:** On startup, HansCorp scans `~/.claude/skills/` for folders containing a `SKILL.md` with valid YAML frontmatter. If `user_invocable: true` is set, the skill's `name` and `description` are registered with Telegram's `setMyCommands` API alongside the built-in commands. Telegram allows up to 100 commands total.
...
When you start HansCorp, a small web page starts running alongside the bot. It reads directly from the same database the bot uses and shows you the data in real time.
...
Open the `.env` file in your HansCorp folder. (This is the same file where your Telegram token and other keys live. Open it with any text editor. TextEdit on Mac, Notepad on Windows, or whatever your terminal editor is.)
...
| **Workspace name** | Rename the sidebar header. Useful if you run multiple HansCorp installs (work / personal). |
...
Requires a Slack User OAuth Token. This connects to your workspace so HansCorp can read and send messages on your behalf.
...
4. Fill in:
   - **App Name**: anything you want (e.g. `HansCorp`)
...
### Step 4: Add the token to HansCorp

1. Open your `.env` file in the HansCorp project directory
2. Add the token:
   ```
...
- You restarted HansCorp after adding the token

### Using Slack from Claude Code (skill)

HansCorp ships with a Slack CLI at `dist/slack-cli.js` and a matching skill in `skills/slack/`. To use Slack via natural language from any Claude Code session:
...
![HansCorp memory system diagram](assets/memory-diagram.png)

HansCorp has a structured memory system that extracts, consolidates, and recalls knowledge across all sessions. Everything is automatic.
...
HansCorp ships with SQLite and **creates everything automatically on first run**: No migrations, no setup, no external database server. File lives at `store/claudeclaw.db`.
...
`CLAUDE.md` is loaded into every Claude Code session. It's the personality and context file. the main thing to edit to make HansCorp yours.
...
```
Create a ~/Business folder structure for my HansCorp agents:
...
HansCorp auto-loads every skill in `~/.claude/skills/`. Install a skill by copying its folder there.

### Bundled skills

HansCorp ships with ready-to-use skills in the `skills/` directory. Copy any of these to activate them:
...
**It's a grey area, but signs point to yes for personal use.** Anthropic's Agent SDK (`@anthropic-ai/claude-agent-sdk`) is a published, official package. Boris Cherny (Anthropic) has indicated the Agent SDK can be used for personal usage with a Claude subscription. HansCorp uses this SDK exclusively.

**How HansCorp works:** The Agent SDK's `query()` spawns the `claude` binary as a child process. That subprocess manages its own auth from `~/.claude/`. HansCorp never reads or transmits your token. It runs Claude Code and reads the output, identical to typing `claude -p "message"` in a terminal.

| | HansCorp | Token-extraction tools |
|---|---|---|
...
HansCorp has multiple security layers. Some are always on, others are opt-in. The setup wizard (`npm run setup`) configures all of them interactively.

### Always on

These protections are active in every HansCorp installation, no configuration needed.
...
Set `EMERGENCY_KILL_PHRASE` to a unique phrase. Sending it immediately stops all HansCorp launchd/systemd services and force-exits the process. This is a hard stop, not a lock. Use it if something goes wrong and you need everything shut down now.
...
- HansCorp runs the real Claude Code CLI, which has full internet access through its built-in tools (web search, web fetch, bash with curl, etc.)
- If Claude says it can't access the internet, it's likely a skill or prompt issue, not a HansCorp limitation
...
**File downloads fail**
- Telegram caps downloads at 20MB. this is a Telegram API limit, not a HansCorp one
...
No. HansCorp has nothing to do with Anthropic's Remote product. It runs the `claude` CLI locally on your own machine (Mac, Linux, or Windows via WSL2) and pipes results to Telegram. No cloud VMs, no remote sessions.

**"Does this work on Windows?"**
Yes, two ways. WSL2 is the smoothest (install WSL2, clone HansCorp inside the WSL filesystem, run the normal Linux setup). Native Windows also works: the setup wizard registers a per-user Scheduled Task at logon (no admin rights), and agent activate/deactivate uses `schtasks` under the hood. War Room voice still needs WSL2 because the Python stack is POSIX-only.
...
Recommended but not required. The video covers how Claude Code works under the hood, which helps you understand what HansCorp is actually doing. But you can set up HansCorp first and watch it later.

**"How do I update when a new version drops?"**
`cd claudeclaw-os && git pull && npm install && npm run migrate && npm run build` then restart. See [Updating HansCorp](#updating-claudeclaw) above.

**"Telegram formatting looks broken / not formatting properly"**
HansCorp converts Claude's Markdown to Telegram-safe HTML (bold, italic, code blocks, links). Telegram's formatting support is limited compared to a full web page. If something looks off, it's usually Telegram's rendering, not a bug. For very long or complex responses, the formatting is intentionally kept simple to avoid Telegram parse errors.
...
This is a core feature, not experimental. Setting up multiple agents is straightforward and one of the most powerful things about HansCorp.
...
**How it works in plain English:** Each agent is just another Telegram bot running the same HansCorp code, but with a different personality file (CLAUDE.md) and a different Telegram token. They all share your machine, your database, and your skills. The main agent can delegate work to specialists, and they ping you back on Telegram when they're done.

HansCorp can run **specialist agents** alongside the main bot. Each agent is its own Telegram bot with its own personality, its own Claude Code session, and its own chat on your phone.
...
Your main HansCorp bot does everything. That's powerful but also means one long conversation, one context window, and one personality trying to handle email, research, billing, and content all at once.
...
Each will show:
```
HansCorp agent [comms] online: @yourname_comms_bot
```
...
To completely remove HansCorp (services, config, database, temp files):
